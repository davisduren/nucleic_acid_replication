...
def order_of_mag(a):
    ...
    order = np.array([len(str(x)) for x in a]) # NOVEL 9/5/24 at gpt 4's suggestion
            #above fixes my vectorize issue from if short had a size of 0 (like during first few iterations)
    return order

def num_nucleo(int_nt_list):
...
def break_long(long, cleav_prop, cleav_prop_struct, mapping):
   ...
   # Calculate the length  of the strand and the number of bonds
        ...
        for no in range(1, order):
            ...
            IT WILL STILL CHOP THEM OFF BY THE NATURE OF DIVISION - DIVISION REMOVES 
            THE LAST NUMBER 
            #part0 = part%10**n_bond
            #part = part//10**n_bond
...
def compute_median_length(strand_length_tracking_list):
    return round(statistics.median(strand_length_tracking_list), 2)



def plot_median_length(median_length_list, output_folder_name, simulation_name):
    """
    Plots the given list of float values.
    X-axis represents iterations (starting from 1).
    Y-axis represents mean_length values.
    """
    iterations = list(range(1, len(median_length_list) + 1))  # Iteration numbers
    
    plt.figure(figsize=(8, 5))  # Set figure size
    plt.plot(iterations, median_length_list, marker='o', linestyle='-', color='b', label='Median Length')  
    
    plt.xlabel("Iteration")
    plt.ylabel("Median Length")
    plt.title("Median Seq Length Throughout Simulation " +str(simulation_name))
    plt.grid(True, linestyle='--', alpha=0.6)  #adds grid for better readability
    plt.legend()
    
    try:
        output_folder = "./feed in strucs overhaul output/" + str(output_folder_name)
        output_file_path = os.path.join(output_folder, f"Median_Strand_Length_{simulation_name}.jpg")
        plt.savefig(output_file_path, dpi=300)
        output_file_path_svg = os.path.join(output_folder, f"Median_Strand_Length_{simulation_name}.svg")
...
def generate_sequence_length_histogram(file_path, folder_name, progress_report_num, simulation_name):
    ...
    plt.title('Sequence Length Histogram' + str(simulation_name) + "___run_" + str(progress_report_num))

    try:
        output_folder = "./feed in strucs overhaul output/" + str(folder_name)
        output_file_path = os.path.join(output_folder, f"{progress_report_num}_sequence_length_histogram_{simulation_name}.jpg")
        plt.savefig(output_file_path, dpi = 300)
        output_file_path_svg = os.path.join(output_folder, f"{progress_report_num}_sequence_length_histogram_{simulation_name}.svg")
        plt.savefig(output_file_path_svg, format="svg")
...
def error_safeguard_system(file_being_written_to, it_num, list_of_sequences):
    try:
        ...
        if isinstance(file_being_written_to, str):
            file_being_written_to = open(file_being_written_to, "a")
        ...
        if not file_being_written_to.closed:
            ...
            for seq in list_of_sequences:
                ...
                file_being_written_to.flush() #if this line is removed the whole function breaks
...
def generate_final_seq_file(directory, final_file_name):
    try:
        ...
        print(f"An error occurred with generating final seq file: {str(e)}")
        return None


    

def generate_line_plot_of_longest_strand(list_w_lengths, output_folder_name, simulation_name, n_iterations):
    ...
    try:
        ...
        output_file_path = os.path.join(output_folder, f"_longest_strand_length_graph_{simulation_name}.jpg")
...
def percentage_plot(list_of_percentages, output_folder_name, simulation_name, len1, len2, len3, len4):
    plt.clf()
    ...
    plt.title("Percentage of strands over a given length over time" + str(simulation_name))
    plt.legend(['>=' + str(len1), '>=' + str(len2), '>=' + str(len3), '>=' + str(len4)])  # Set labels for the legend

    try:
        output_folder = "./feed in strucs overhaul output/" + str(output_folder_name)
        output_file_path = os.path.join(output_folder, f"Percentage_strands_over_lengthX_{simulation_name}.jpg")
        plt.savefig(output_file_path, dpi=300)
        output_file_path_svg = os.path.join(output_folder, f"Percentage_stands_over_lengthX_{simulation_name}.svg")
        plt.savefig(output_file_path_svg, format="svg")
...
def percentage_nt_plot(list_of_percentage_nt_involved_in_structure,output_folder_name, simulation_name):
    plt.clf()
    ...
    try:
        output_folder = "./output/" + str(output_folder_name)
        output_file_path = os.path.join(output_folder, f"Nt_percentage_{simulation_name}.jpg")
...
#LAST UPDATE: 2/19/25 - changing output to be based off of parameters, not time run
...
base_directory = "./feed in strucs overhaul output/" #output folder that stuff will be saved into
simulation_name = str(cleav_prop) + "-" + str(cleav_prop_struct) + " - " + init_seq_file[:-4] #shows the important parameters of the simulation
...
if throwing_long_ones_out == True:
    ...
    with open(seqs_over_certain_length_filepath, "a") as seqs_over_certain_length_write:
        try:
            ...
            seqs_over_certain_length_write.flush()  
        except Exception as e:
...
str_nt_list = [s.strip() for s in list_of_starting_seqs]

str_nt_list = np.array(str_nt_list)

mapping = {"1": "A", "2": "G", "3": "C", "4": "U"}


#Need to convert str_nt_list back into list of ints for below operations
...
for it in range(1, n_iterations + 1):
    ...
    order = order_of_mag(int_nt_list).astype(object)
    mono = int_nt_list[order == 1]
    short = int_nt_list[np.logical_and(order > 1, order < length_threshold)]
    long = int_nt_list[order >= length_threshold]

    for strand in long:

        # Remove the strands from long that fail the ∆G test and paste them into short
        ...
        if throwing_long_ones_out == True:
            ...
            if len(str(strand)) >= length_to_throw_out: # means we want to throw that strand out
                ...
                if dg_value <= dG_critical_value: # means it is more negative than our dG_crit
                    indices_to_remove = np.where(int_nt_list == strand)[0]
                    list_of_seqs_over_length_threshold.append(strand) # see "if it%n_iterations ==0:" line
                    int_nt_list = np.delete(int_nt_list , indices_to_remove) 
                    stringified_nt_list = map(str, int_nt_list)
        ...
        if dg_value <= dG_critical_value: #means its stable & it stays in long
            ...
            count = np.sum(long == strand)
            long = long[long != strand] #removes EVERY instance within long of this exact strand
            short = np.append(short, [strand] * count)
    ...
    median_len_this_it = compute_median_length(strand_length_tracking_list)
    median_lengths.append(median_len_this_it)
    ...
    longest_strand_str = str(strands_in_decreasing_order[0])
    ...
    if it%n_iterations == 0: 
        ...
        summary = np.sort(int_nt_list)[::-1] #to get all lengths, remove [::-1][-:10]
        ...
        # NOVEL 2/19/25
        # important data from the simulation (for use in making the heat map)
            
        important_data_file_name = f"important_data_{folder_name}.txt"   
        important_data_file_path = generate_final_seq_file(output_directory, important_data_file_name)
        ...
        summary = np.sort(int_nt_list)[::-1] # [::-1] = longest --> shortest
        #print(summary)
        summary_s = convert_int_to_str_seq(summary, mapping)
        ...
        generate_sequence_length_histogram(file_path, folder_name, it, simulation_name)




#generate_sequence_length_histogram(final_file_path, folder_name, n_iterations)
#generate_line_plot_of_longest_strand(longest_strand_length_list, folder_name, simulation_name, n_iterations)
#print(list_of_percent_lens)
percentage_plot(list_of_percent_lens, folder_name, simulation_name, len1, len2, len3, len4)
#percentage_nt_plot(list_of_percentage_nt_involved_in_structure, folder_name, simulation_name)
plot_median_length(median_lengths, folder_name, simulation_name)



"""
# of 2˚ structures and see populations of 2˚ structures

dump in something each iteration ? be interestingto see what would happen

#DO THIS put known hairpins in and see the half life of them - search sequence space, continuous evolution

main result: selection on the basis of degradation, not synthesis


2D plot. x-axis is unstruc cleavage rate
y1 is struc cleavage rate
y2 is MEDIAN Length



Show persistence - how?
perhaps track the sequences 
